...
public class MemberServiceImpl implements MemberService { // CURD : Create(생성), Read(읽기), Update(갱신), Delete(삭제)
	...
	public List<MemberVO> memberSelectList() {
		...
		try { // 여기에 DB 처리
			...
			rs = psmt.executeQuery(); // select // update엔 executeUpdate
			while (rs.next()) { // true인 동안 // 안에 데이터가 있을 동안
				vo = new MemberVO();
	...
	public MemberVO memberSelect(MemberVO vo) {
		...
		try { // 여기에 DB 처리
			...
			rs = psmt.executeQuery(); // select // update엔 executeUpdate
			if (rs.next()) { // true인 동안 // 안에 데이터가 있을 동안
				vo = new MemberVO();
				vo.setMemberId(rs.getString("member_id"));
				vo.setMemberPw(rs.getString("member_pw"));
				vo.setMemberName(rs.getString("member_name"));
